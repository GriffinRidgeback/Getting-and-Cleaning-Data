...
# Print the results
message("Number of rows: ", nrow(ordereddata))
message("13th entry: ", ordereddata[13, "Full"])

# Calculate average GDP ranking
ranking <- tapply(ordereddata$Rank, ordereddata$Income.Group, mean, simplify = F)

# Print the results
message("Avg GDP rank for High Income: OECD   : ", ranking$`High income: OECD`)
message("Avg GDP rank for High Income: nonOECD: ", ranking$`High income: nonOECD`)
 
# Answer for question 5
message("The answer is 5; only other possible choice would be 12 because the first time there was")
message("a zero and a 13; I took 13, which was wrong; next time 0 ~ 1, and 13 ~ 18")
message("When you take the course again, ask the question about why you got the 189 right but not the last bin right (37 but not 38)")
# Here are the commands I used to get the answer 4
# copy_data  <- ordereddata
# copy_data$gdp_rank  <- cut2(copy_data$GDP, g=5)
# table(copy_data$Income.Group, copy_data$gdp_rank)